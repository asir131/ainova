...
export async function POST(request) {
  try {
    ...
    // Create a transporter with robust settings for any SMTP provider
    const transporter = nodemailer.createTransport({
      host: process.env.SMTP_HOST,
      port: parseInt(process.env.SMTP_PORT || "587", 10),
      secure: process.env.SMTP_PORT === "465", // Use `true` for port 465 (SSL/TLS), `false` for other ports (STARTTLS)
      auth: {
